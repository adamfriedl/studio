...
func PRReview(in PRReviewInput) string {
	...
	fmt.Fprintf(&b, "Shape the review like a compact PR Reviewer Guide for a human (and for the implementer via inline comments):\n")
	fmt.Fprintf(&b, "- estimated review effort 1–5\n")
	fmt.Fprintf(&b, "- whether the PR contains/updates tests (yes | no | n/a)\n")
	fmt.Fprintf(&b, "- one-line security blurb\n")
	fmt.Fprintf(&b, "- up to 3 ranked focus areas with why (these may overlap inline comments)\n\n")
	fmt.Fprintf(&b, "Reply with exactly:\n")
	fmt.Fprintf(&b, "verdict: lgtm | changes-requested\n")
	fmt.Fprintf(&b, "effort: 1-5\n")
	fmt.Fprintf(&b, "tests: yes | no | n/a\n")
	fmt.Fprintf(&b, "security: one short line\n")
	fmt.Fprintf(&b, "summary: one short paragraph\n")
	fmt.Fprintf(&b, "focus:\n")
	fmt.Fprintf(&b, "- area: short title\n")
	fmt.Fprintf(&b, "  why: one sentence rationale\n")
	...
	fmt.Fprintf(&b, "If the change matches the issue and residual risk is small, verdict lgtm (effort still required; focus may be empty). Prefer a small mergeable PR over a perfect one. Put concrete defects in comments so the implementer can act; use focus for human guidance.\n")
	return b.String()
}
